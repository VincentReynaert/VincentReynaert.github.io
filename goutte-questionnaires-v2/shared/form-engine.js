...
import { findRosterMatches, getRosterPrefixMatches } from './roster.js';
...
async function attachConsentPidResolver(form) {
  ...
  async function refreshPid() {
    ...
    try {
      let matches = await findRosterMatches(lastName, firstName);
      if (!matches.length) matches = await getRosterPrefixMatches(lastName, firstName);
      const prefix = buildPidPrefix(lastName, firstName);

      if (!matches.length) {
        showMessage(message, 'error', `Aucun identifiant trouvé dans la base pour ${prefix}. Ajoutez cet étudiant dans data/roster.json.`);
        return;
      }

      if (matches.length === 1) {
        pidInput.value = matches[0].pid;
        showMessage(message, 'success', `Identifiant reconnu : ${matches[0].pid}`);
        return;
      }
